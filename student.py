...
# Testing Connection 
"""
conn = mysql.connector.connect(username='root', password='Krishnamkmk4422@',host='localhost',database='krish')
cursor = conn.cursor()
...
class Student:
    ...
    # This part is image labels setting start
        # first header image  
        img = Image.open(
            r"C:\Users\admin\Desktop\Face reg\GUI Images\banner1.jpg")
        ...
        bg1 = Image.open(
            r"C:\Users\admin\Desktop\Face reg\GUI Images\face-off-banner.jpg")
        ...
        # title section
        title_lb1 = Label(bg_img, text="Face Recognition Attendance Management System ", font=("verdana", 30, "bold"),
                          bg="black", fg="gray")
        ...
        dep_combo["values"] = ("Select Department", "BSC", "BA", "B COM")
        dep_combo.current(0)
        ...
        cou_combo = ttk.Combobox(current_course_frame, textvariable=self.var_course, width=15,
                                 font=("verdana", 12, "bold"), state="readonly")
        cou_combo["values"] = ("Select Course", "Computer", "IT", "Maths", "English", "Tamil")
        cou_combo.current(0)
        ...
        year_combo = ttk.Combobox(current_course_frame, textvariable=self.var_year, width=15,
                                  font=("verdana", 12, "bold"), state="readonly")
        year_combo["values"] = ("Select Year", "2017-20", "2018-21", "2019-22", "2020-23", "2021-24")
        ...
        # Class Didvision
        student_div_label = Label(class_Student_frame, text="Class Shift:", font=("verdana", 12, "bold"),
        ...
        # Roll No
        student_roll_label = Label(class_Student_frame, text="Roll_No:", font=("verdana", 12, "bold"), fg="navyblue",
                                   bg="white")
        ...
        # Phone Number
        student_mob_label = Label(class_Student_frame, text="Mobile_No:", font=("verdana", 12, "bold"), fg="navyblue",
                                  bg="white")
        ...
        radiobtn1 = ttk.Radiobutton(class_Student_frame, text="Take Photo Sample", variable=self.var_PhotoSample,
                                    value="Yes")
        radiobtn1.grid(row=5, column=0, padx=5, pady=5, sticky=W)

        radiobtn1 = ttk.Radiobutton(class_Student_frame, text="No Photo Sample", variable=self.var_PhotoSample, value="No")
        ...
        take_photo_btn = Button(btn_frame, command=self.generate_dataset, text="Take Pic", width=9,
                                font=("verdana", 12, "bold"), fg="white", bg="gray")
        ...
        # Phone Number
        search_label = Label(search_frame, text="Search:", font=("verdana", 12, "bold"), fg="gray", bg="white")
        ...
        "ID", "Name", "Dep", "Course", "Year", "Sem", "Div", "Gender", "DOB", "Mobile_No", "Address", "Roll_No", "Email", "Photo"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        ...
        self.student_table.column("Roll_No", width=40)
    ...
    def add_data(self):
        if self.var_dep.get() == "Select Department" or self.var_course.get == "Select Course" or self.var_year.get() == "Select Year" or self.var_semester.get() == "Select Semester" or self.var_std_id.get() == "" or self.var_std_name.get() == "" or self.var_div.get() == "" or self.var_roll.get() == "" or self.var_gender.get() == "" or self.var_dob.get() == "" or self.var_email.get() == "" or self.var_mob.get() == "" or self.var_address.get() == "" :
            ...
            try:
                ...
                my_cursor.execute("INSERT INTO stud values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (
                self.var_std_id.get(),
    ...
    def fetch_data(self):
        conn = mysql.connector.connect(host="localhost", username="root", password="Krishnamkmk4422@", database="krish")
        my_cursor = conn.cursor()
        my_cursor.execute("select * from stud")
        ...
        if self.var_dep.get() == "Select Department" or self.var_course.get == "Select Course" or self.var_year.get() == "Select Year" or self.var_semester.get() == "Select Semester" or self.var_std_id.get() == "" or self.var_std_name.get() == "" or self.var_div.get() == "" or self.var_roll.get() == "" or self.var_gender.get() == "" or self.var_dob.get() == "" or self.var_email.get() == "" or self.var_mob.get() == "" or self.var_address.get() == "" :
            ...
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(host="localhost", username="root", password="Krishnamkmk4422@", database="krish")
                    my_cursor = conn.cursor()
                    my_cursor.execute("update stud set Name=%s,Department=%s,Course=%s,Year=%s,Semester=%s,Division=%s,Gender=%s,DOB=%s,Mobile_No=%s,Address=%s,Roll_No=%s,Email=%s,PhotoSample=%s where Student_ID=%s",
                        (
                            self.var_std_name.get(),
                            self.var_dep.get(),
                            self.var_course.get(),
        ...
        if self.var_std_id.get() == "":
            ...
            try:
                ...
                if delete > 0:
                    conn = mysql.connector.connect(host="localhost", username="root", password="Krishnamkmk4422@", database="krish")
                    my_cursor = conn.cursor()
                    sql = "delete from stud where Student_ID=%s"
    ...
    def search_data(self):
        if self.var_search.get() == "" or self.var_searchTX.get() == "Select":
            ...
            try:
                conn = mysql.connector.connect(host="localhost", username="root", password="Krishnamkmk4422@", database="krish")
                my_cursor = conn.cursor()
                sql = "SELECT Name,Department,Course,Year,Semester,Division,Gender,DOB,Mobile_No,Address,Roll No,Email,PhotoSample FROM stud where Student_ID" + str(
                    self.var_search.get()) + ""
            ...
            try:
                ...
                while True:
                    ...
                    if face_cropped(my_frame) is not None:
                        ...
                        file_path = "data_img/stud." +str(id)+'.'+ str(img_id) + ".jpg"
